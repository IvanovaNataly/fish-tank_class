...
class App {
    constructor () {
        ...
        this.pictureGold = "img/babelfish.png";
        let pictureCat = "img/catfish.png";
        let goldMax = 5;
        let catMax = 7;
        this.tank = new Tank($container, this.pictureGold, pictureCat, goldMax, catMax);
    }
}

class Tank {
    constructor ($container, pictureGold, pictureCat, goldMax, catMax) {
        this.$container = $container;
        this.pictureGold = pictureGold;
    ...
    checkFishType (fishType) {
        if (fishType.includes('add-btn')) {
            fishType = this.pictureGold;
            this.addFish(fishType);
        } else {
            fishType = this.pictureCat;
    ...
    feed () {
        ...
        for (let i = this.fishArr.length-1; i >= 0; i--) {
            if (i >= (this.fishArr.length-this.goldMax)) {
                this.fishArr[i].eat();
                this.dirt();
            } else {
